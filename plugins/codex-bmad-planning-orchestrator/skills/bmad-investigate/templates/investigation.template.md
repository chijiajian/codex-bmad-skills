...
> Proceed with `$bmad-epics-and-stories` to compile the
...
> Proceed with `$bmad-epics-and-stories` (Update intent).
...
- [ ] `$bmad-architecture` (Update intent) — if a design
      decision needs revision.
- [ ] `$bmad-prd` (Update intent) — if a requirement is
      missing or ambiguous.
- [ ] `$bmad-readiness-check` — if multiple areas are at risk.
