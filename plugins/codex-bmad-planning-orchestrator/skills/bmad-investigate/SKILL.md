...
```
../bmad-investigate/templates/investigation.template.md
```
...
that the user run `$bmad-prd` or
`$bmad-architecture` (Update intent) before a story
is created.
...
- `$bmad-epics-and-stories` — creates the fix story
- `$bmad-readiness-check` — if investigation reveals
  systemic planning gaps before the fix story is safe to hand off

> ---
> Part of the **BMAD Planning & Orchestrator** plugin — a Codex harness for the **BMAD Method** by the **BMAD Code Organization** (https://github.com/bmad-code-org/BMAD-METHOD). Implements the spirit of `bmad-investigate`. All methodology credit belongs to the BMAD Code Organization.
