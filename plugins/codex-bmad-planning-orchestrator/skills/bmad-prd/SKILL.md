...
7. **Emit** — fill `../bmad-prd/templates/prd.template.md` → `prd.md`. Park overflow in `addendum.md` via `../bmad-prd/templates/addendum.template.md`. Append decisions to `decision-log.md`.
...
### VALIDATE
1. Run `bash ../bmad-prd/scripts/validate-prd.sh bmad-output/prd.md`.
...
- **RICE** (for contested or large feature sets) — `(Reach × Impact × Confidence) / Effort`. Run the helper:
  ```
  python3 ../bmad-prd/scripts/prioritize.py              # interactive
  python3 ../bmad-prd/scripts/prioritize.py -b feats.csv # batch (name,reach,impact,confidence,effort)
  ```
...
> ---
> Part of the **BMAD Planning & Orchestrator** plugin — a Codex harness for the **BMAD Method** by the **BMAD Code Organization** (https://github.com/bmad-code-org/BMAD-METHOD). Implements the spirit of `bmad-prd`. All methodology credit belongs to the BMAD Code Organization.
