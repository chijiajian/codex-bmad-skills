...
Both scripts are deterministic and read-only. Resolve the referenced script paths from
the installed skill directory before running them.

```bash
# 1) Build the dependency DAG (edges + conflict class) from status + story scopes
python3 "../bmad-parallel-plan/scripts/build-dependency-graph.py" \
...
python3 "../bmad-parallel-plan/scripts/plan-parallel-waves.py" \
...
> ---
> Part of the **BMAD Planning & Orchestrator** plugin — a Codex harness for the **BMAD Method** by the **BMAD Code Organization** (https://github.com/bmad-code-org/BMAD-METHOD). Implements the spirit of `bmad-parallel-plan`. All methodology credit belongs to the BMAD Code Organization.
