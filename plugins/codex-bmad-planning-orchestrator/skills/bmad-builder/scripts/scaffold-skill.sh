...
Creates:
  ...
  skills/<skill-name>/agents/openai.yaml (Codex UI metadata)
...
echo -e "${GREEN}Created: $TARGET_DIR/agents/${NC}"
...
description: |
  ...
  phrases ("Use when the user says ...") so Codex can invoke this skill.
...
Resolve bundled resources relative to this skill directory. When running a bundled script, use the absolute path to that script from the installed plugin location; relative examples are shown from this \`SKILL.md\` directory.
...
1. Run the validator:
   \`\`\`bash
   bash \../${SKILL_NAME}/scripts/validate-output.sh bmad-output/<output-file>.md
   \`\`\`
...
> ---
> Part of the **BMAD Planning & Orchestrator** plugin — a Codex harness for the **BMAD Method** by the **BMAD Code Organization** (https://github.com/bmad-code-org/BMAD-METHOD). Implements the spirit of \`bmad-TODO\`. All methodology credit belongs to the BMAD Code Organization.
SKILLEOF

echo -e "${GREEN}Created: $TARGET_DIR/SKILL.md${NC}"

cat > "$TARGET_DIR/agents/openai.yaml" <<AGENTEOF
interface:
  display_name: "BMAD ${DISPLAY_TITLE}"
  short_description: "Help with BMAD planning workflows"
  default_prompt: "Use \$${SKILL_NAME} to work through this BMAD planning task."
AGENTEOF

echo -e "${GREEN}Created: $TARGET_DIR/agents/openai.yaml${NC}"
...
echo "  2. Edit $TARGET_DIR/agents/openai.yaml if the UI labels need refinement"
echo "  3. Add skill-specific scripts to $TARGET_DIR/scripts/"
echo "  4. Add planning document templates to $TARGET_DIR/templates/"
echo "  5. Optionally create REFERENCE.md for details (keeps SKILL.md under 5K tokens)"
echo "  6. Validate:"
echo "     bash \../bmad-builder/scripts/validate-skill.sh \\"
